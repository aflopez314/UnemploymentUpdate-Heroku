...
def update_install():
   con = pg.connect(DATABASE_URL, sslmode='require')
   cur = con.cursor(cursor_factory=pge.RealDictCursor)
   cur.execute("SELECT * FROM jobsapplied ORDER BY AppDate::DATE DESC")
   rows = cur.fetchall()
...
@app.route('/updaterec_rejected', methods= ['GET', 'POST'])
def updaterec_rejected():
   if request.method=='POST':
      try:
         data = request.get_json()
         Site = data.get('Site')
         ...
         with pg.connect(DATABASE_URL, sslmode='require') as con:
            cur = con.cursor()
            cur.execute("UPDATE jobsapplied SET Response='Rejected' WHERE Site=%s AND Position=%s", (Site,Position))
            con.commit()
...
@app.route('/updaterec_affirmative', methods= ['GET', 'POST'])
def updaterec_affirmative():
   if request.method=='POST':
      try:
         data = request.get_json()
         Site = data.get('Site')
         ...
         with pg.connect(DATABASE_URL, sslmode='require') as con:
            cur = con.cursor()
            cur.execute("UPDATE jobsapplied SET Response='Affirmative' WHERE Site=%s AND Position=%s", (Site,Position))
            con.commit()
            msg = "Record successfully updated"
      except:
         msg = "error in update operation"
